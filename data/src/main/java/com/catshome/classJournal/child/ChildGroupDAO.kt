...
//    @Query("select c.uid as childUid , c.child_name as childName, " +
//            "c.child_surname as childSurname,c.child_birthday as childBirthday , " +
//            "g.group_name as groupName, g.uid as groupUid  from child as c  left " +
//            "join   'child_group' as c_g on c_g.childId =c.uid left join  `groups` as g on " +
//            "groupId = g.uid or groupId =null and c.isDelete =:isDelete")
    @Query("select c.uid as childUid , c.child_name as childName, c.child_surname as childSurname,c.child_birthday as childBirthday , g.group_name as groupName, g.uid as groupUid  from child as c  left join   'child_group' as c_g on c_g.childId =c.uid left join  `groups` as g on groupId = g.uid or groupId =null WHERE c.isDelete =:isDelete")
