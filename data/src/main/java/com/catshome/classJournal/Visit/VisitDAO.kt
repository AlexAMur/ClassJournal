...
interface VisitDAO {
    ...
    @Query(
        "Select v.uid , v.uidChild , c.child_name as Name, c.child_surname as Surname, v.dateVisit " +
                ", v.priceVisit  from visits v join child c where v.uidChild =c.uid and c.isDelete = '0' ORDER BY " +
                " CASE WHEN :sortDate = 'dateVisit' THEN  dateVisit END DESC," +
                " CASE WHEN :sortDate = 'dateVisit' THEN  Surname END ASC ," +
                " CASE WHEN :sortDate = 'dateVisit' THEN  name END ASC ," +
                " CASE WHEN :sortSurname = 'Surname' THEN Surname END ASC,"+
                " CASE WHEN :sortSurname = 'Surname' THEN name END ASC,"+
                " CASE WHEN :sortSurname = 'Surname' THEN dateVisit END DESC"
    )
    fun getFullVisit(
        sortDate: String,
        sortSurname: String
    ...
    @Query(
        "Select v.uid , v.uidChild , c.child_name as Name, c.child_surname as Surname, v.dateVisit, " +
                "v.priceVisit  from visits v join child c where uidChild =c.uid and v.uidChild= :uidChild ORDER BY " +
                "dateVisit DESC"
    )
    ...
    @Query(
        "Select v.uid  , uidChild , c.child_name as Name, c.child_surname as Surname, dateVisit, " +
                "priceVisit  from visits v join child c where uidChild =c.uid and uidChild= :uidChild and " +
                "dateVisit>= :begin and dateVisit <= :end ORDER BY  dateVisit  DESC"
    )
...
//    )
    ...
    @Query(
        "select c.uid as uidChild,s.dayOfWeek, s.startlesson, null as groupName, c.saldo, " +
                "(c.child_name||' '||c.child_surname) as fio, 0 as 'check', s.price  from child c " +
                "Join scheduler s where c.uid = s.uidChild and uidChild !='null' union " +
                "select c.uid as uidChild, s.dayOfWeek, " +
                "s.startlesson, g.group_name as groupName, c.saldo ,(c.child_name||' '||c.child_surname) as fio, " +
                "0 as 'check', s.price from child c Join child_group cg Join `groups` g join scheduler s " +
                "where c.uid = cg.childId and g.uid = cg.groupId  and cg.groupId in (select uidGroup from " +
                "scheduler where uidGroup !='null') and s.uidGroup = g.uid " +
                "order by dayOfWeek,startLesson"
